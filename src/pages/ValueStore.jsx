...
import { valueStoreData } from "../data/valueStore";
...
const ValueStore = () => {
  ...
  const companyHandler = (valueStore) => {
    if (selectedProduct.includes(valueStore)) {
      setSelectedProduct(selectedProduct.filter((item) => item !== valueStore));
    } else {
      setSelectedProduct([...selectedProduct, valueStore]);
    }
  };
  ...
  const filteredProduct =
    selectedProduct.length === 0
      ? valueStoreData
      : valueStoreData.filter((valueStore) =>
          selectedProduct.includes(valueStore.brand)
        );
        ...
        <Stack direction="horizantal" gap={"50px"}>
          <Stack width="25%" textAlign="left">
            ...
            <Stack
              ...
              paddingBottom="10px"
            >
              <Text fontSize="14px">VALUE STORE</Text>
            ...
            {valueStoreData.map((medicine, index) => (
              <Stack
                direction="horizontal"
                justify="space-between"
                align="center"
              ...
              >
                ...
                <Checkbox
                  isChecked={selectedProduct.includes(medicine.brand)}
                  onChange={() => companyHandler(medicine.brand)}
                  colorScheme="teal"
                  size="lg"
                />
              </Stack>
          ...
          <Stack width="75%">
            ...
            <Grid templateColumns="repeat(3,0.5fr)" gap="5">
              {filteredProduct.map((item) => (
                <Link key={item.id} to={`/value-store/${item.id}`}>
                  <Box
                    height="330px"
                    width="230px"
                    padding="10px"
