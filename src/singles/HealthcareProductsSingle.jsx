...
const HealthcareProductsSingle = () => {
  const { id } = useParams();

  // Find the product based on the id parameter
  const product = healthcareProductsData.find((item) => item.id === id);

  // Check if product is found
  if (!product) {
    return <div>Product not found</div>; // Render a message or redirect to a 404 page
  }
...
export default HealthcareProductsSingle;
